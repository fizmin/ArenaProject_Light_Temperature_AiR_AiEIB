...
void setup() {
  ...
  // ====================== Inicjalizacja radia LoRa ======================
    ...
    // ====================== Pobranie pomiarów ======================
    float temperature = bme.readTemperature() - 4; // Pomiar temperatury (z offsetem -4°C)
    ...
    // ====================== Wysyłanie danych ======================
    sendHybridProtected(BASE_STATION_ADDRESS, (uint8_t *)payload, 15);
...
// Funkcja wysyłania hybrydowego: bezpośrednio i przez router
// dest = 4, router = 0
void sendHybridProtected(uint8_t dest, uint8_t *payload, uint8_t size) {
  // zapamiętaj oryginalny wpis
  int oldNext = routing_table[dest];

  // 1. próbuj bezpośrednio
  routing_table[dest] = dest;
  bool okDirect = Lora::send_with_ack(dest, payload, size);
  Serial.println(okDirect ? "Direct OK" : "Direct FAIL");

  delay(80); // mała przerwa; można zmienić

  // 2. spróbuj przez router
  routing_table[dest] = 0;
  bool okRouter = Lora::send_with_ack(dest, payload, size);
  Serial.println(okRouter ? "Via router OK" : "Via router FAIL");

  // przywróć
  routing_table[dest] = oldNext;
}
